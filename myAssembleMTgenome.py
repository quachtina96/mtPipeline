...
if ext=='sam':
	...
	cmd='%s view -bt %s.fai %s > %s' %(sexe,mtdnafile,samfile,bamfile) #Tina: changed the index ref from whole genome to mt genome
	os.system(cmd)
	ext='bam'
if ext=='bam':
	print 'Sorting and indexing BAM...'
	cmd1='%s sort %s.bam %s-sorted' %(sexe,basext,basext) #Tina:  changed the index ref from whole genome to mt genome
