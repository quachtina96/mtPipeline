...
path = "/gpfs/home/quacht/debug/ID18_Father"
depths = []
...
#job = sp.Popen(command, stdout=sp.PIPE, shell=True)
...
for subdir, dirs, files in os.walk(path):
	for file in files:  # for each file
		...
		if (file.endswith("exome.bam")):
			...
			stdout=sys.stdout
			depthAnalysis = open(str(sampleName + "depthAnalysis.txt"), "w+")
...
for subdir, dirs, files in os.walk(path): 
	for file in files:
		if (file.endswith("csort.bam")):
			...
			stdout=sys.stdout
			sys.stdout=open(str(sampleName + "depth.txt"), "w+")			
			print "Results:"
			print "After Remapping:"
			pf.analyzeCoverage(file, depths)
			sys.stdout=stdout
			depthAnalysis.close()
response = raw_input("Are you satisfied with results and ready to clean up? (y/n)")
if (response == "y"):
	clean(path)
