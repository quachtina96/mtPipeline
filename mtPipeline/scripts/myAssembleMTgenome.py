...
# in order to generate a pileup file that can be used to assemble the MT
# genome and later variant call.
...
try:
	opts, args = getopt.getopt(
		sys.argv[1:], "hf:i:q:c:d:o:g:a:r:s:FCUPNA:D:z:t:")
except getopt.GetoptError, err:
	print str(err)
	usage()
	sys.exit()
fasta_dir = "/gpfs/home/quacht/data/"  
mtdna_fasta = "chrRCRS.fa"
inputfile = ''
hgenome_fasta = "/gpfs/group/stsi/genomes/GATK_bundle/hg19/ucsc.hg19.fasta"
mqual = 25
clev = 0.80
cov = 5
glen = 10
...
hgenome = hgenome_fasta
print mtdnafile
print hgenome

# The sample name is derived from the current directory (which should be
# the sample directory e.g. ID18_Father)
...
dIUPAC = {'AG': 'R', 'CT': 'Y', 'GC': 'S', 'AT': 'W', 'GT': 'K',
		  'AC': 'M', 'CGT': 'B', 'AGT': 'D', 'ACT': 'H', 'ACG': 'V'}
...
if ext == 'sam':
	...
	# Tina: changed the index ref from whole genome to mt genome since we are working with the mtExtract
	cmd = '%s view -bt %s.fai %s > %s' % (sexe, mtdnafile, samfile, bamfile)
	os.system(cmd)
	ext = 'bam'
if ext == 'bam':
	...
	cmd3 = '%s mpileup -B -f %s %s-sorted.bam > %s.pileup' % (
		sexe, mtdnafile, basext, basext)
	print cmd3
	os.system(cmd3)
...
for i in range(len(mtdna)):
	# print i+1, mtdna[i+1]
	line = [str(i+1), mtdna[i+1][0], mtdna[i+1][1][0], str(mtdna[i+1]
														   [1][2]), "%.2f" % (mtdna[i+1][1][3]), str(mtdna[i+1][1][1])]
	f.write('\t'.join(line)+'\n')
	# aseq+=mtdna[i+1][1][0]
	# if variant is not #, contigs will have reference, otherwise the # that
	# will be subsequently substituted with N
	if mtdna[i+1][1][0] != '#':
		aseq += mtdna[i+1][0]
	else:
		aseq += mtdna[i+1][1][0]
...
#
# so that each dict_seq can be handled with the Consensus dict information
# for ambiguities and indels.

# SAMFILE, MT-TABLE FOR MTVCF_GENERATOR.
# Sample name is defined as sample_name =
# os.getcwd().split('/')[-1].split('_')[1]
sam_handle = basext+'.sam'
mt_table_handle = tablefile

# Tina: open the file with no read group
sam_file = open(basext+'.norg.sam', 'r')
mt_table = open(tablefile, 'r').readlines()
mut_events = mtvcf_main_analysis(
	mt_table, sam_file, str(sample_name), tail=tail)
if os.path.exists('VCF_dict_tmp'):
	VCF_dict = ast.literal_eval(
		open('VCF_dict_tmp', 'r').read())  # global VCF dict
else:
		...
		for p_info in consensus_single:
			if p_info[0] in dict_seq.keys():
				...
				if p_info[-1] == 'mism':
					...
					# in the consensus, the ins is reported as the nuc of pos
					# of the ins + the inserted bases
...
if crc:
	f.write(
		'Coverage of Assembled mtDNA. Coverage %.4f - Per base depth %.3f.\n' % (passb, covmt))
if crc:
